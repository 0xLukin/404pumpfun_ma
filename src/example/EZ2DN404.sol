 ...
 */
contract EZ2DN404 is DN404, Ownable {
    ...
    uint256 public mintPriceInterval;
    ...
    constructor(
        ...
        mintPriceInterval = _mintPriceInterval;
    ...
    modifier checkPriceAndUpdate(uint256 price, uint256 nftAmount) {
        uint256 totalMintPrice =
            price * nftAmount + nftAmount * (nftAmount - 1) * mintPriceInterval / 2; // Sn = a1*n + n(n-1)d/2

        if (totalMintPrice < msg.value) {
            revert InvalidPrice();
        }

        // update mintPrice
        uint256 newMintPrice = price + nftAmount * mintPriceInterval;
        mintPrice = newMintPrice;

        // refund the rest
        uint256 restFund = msg.value - totalMintPrice;
        if (restFund > 0) {
            SafeTransferLib.safeTransferETH(msg.sender, restFund);
        }
        _;
    }
    ...
    //     payable
    //     onlyLive
    //     checkPriceAndUpdate(publicPrice, nftAmount)
    ...
    //     payable
    //     onlyLive
    //     checkPriceAndUpdate(allowlistPrice, nftAmount)
    //     checkAndUpdateBuyerMintCount(nftAmount)
    //     checkAndUpdateTotalMinted(nftAmount)
    // {
    ...
    function publicMint(uint256 nftAmount)
        ...
        payable
        onlyLive
        checkPriceAndUpdate(mintPrice, nftAmount)
        checkAndUpdateBuyerMintCount(nftAmount)
        checkAndUpdateTotalMinted(nftAmount)
    {}

    function whitelistMint(uint256 nftAmount, bytes32 signature) public onlyLive {
        // check signature
    }
